...
def test_organization_folder_naming():
    ...
    def get_safe_name(organization_name):
        ...
        if not organization_name:
            return "organization"
        cleaned = re.sub(r'[<>:"/\\|?*]', '', organization_name)
    ...
    expected = [
        "مدارس_الرياض_الأهلية",
        "مدرسة_النور_الأهلية",
        "معهدالبيانالتعليمي",
        "أكاديمية_النجاح_للتعليم",
        "مجمع_المستقبل_التعليمي"
    ]

    for name, exp in zip(test_names, expected):
        safe_name = get_safe_name(name)
        print(f"الاسم الأصلي: {name}")
        print(f"الاسم الآمن: {safe_name} (متوقع: {exp})")
        assert safe_name == exp, f"Expected {exp}, got {safe_name}"
        print("---")
...
def main():
    ...
    print("بدء اختبار نظام المؤسسة والنسخ الاحتياطيات")
    print("=" * 50)
    ...
    try:
        ...
        print(f"النسخ الاحتياطيات: {'✅ نجح' if test3 else '❌ فشل'}")
